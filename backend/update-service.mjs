...
const ASSETS={darwin:'Nodus-mac-arm64.dmg',win32:'Nodus-windows-x64.exe',linux:'Nodus-linux-x64.AppImage'};
...
function validAsset(asset,tag,name){
  return asset?.name===name && asset.browser_download_url===expectedUrl(tag,name) &&
...
export async function checkForUpdate(currentVersion,platform,request=fetch){
  ...
  if(!assetName)throw new Error('当前平台暂不支持自动检查更新');
  ...
  return {
    currentVersion,latestVersion,available,downloadable,
...
export async function downloadUpdate(release,directory,{request=fetch,onProgress=()=>{}}={}){
  if(!release.downloadable||!validAsset({name:release.assetName,browser_download_url:release.assetUrl,size:release.assetSize},release.tag,release.assetName)||release.checksumUrl!==expectedUrl(release.tag,'SHA256SUMS.txt'))throw new Error('更新包信息无效');
