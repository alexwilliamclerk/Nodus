...
export function decisionPrompt(task, previousOptions = []) {
  ...
  return `你是结构化任务的决策助手，只分析，不修改文件。先理解用户意图、否定语句、交付格式、材料与约束，再确定主产物。不要仅匹配关键词。例如“不要网站，给我 PPTX”是 presentation；用 Python 分析数据以结论为交付是 analysis，交付可复用脚本是 python。混合需求需要明确主产物和附属文件；无法判断、要求不支持的格式（例如必须 DOCX/XLSX）或超出协议能力时返回 clarification，不得降级成网站。
可用协议：${JSON.stringify(artifactTypes)}
${context(task)}
...
需澄清时：{"artifactType":"已识别的类型或 null","clarification":"具体需要补充或确认的问题","options":[]}。
...
export function revisionPrompt(task,proposal,label) {
  return `${artifactPrompt(task,label)}\n当前目录是上一可用版本完整副本。保持同一产物类型及格式，只修改已确认范围。\n${JSON.stringify(proposal)}`;
}
